...
Our specific queues network is been modelled using an abstract class called \texttt{System}, which provide all required methods implementations in order to perform our simulation. 

The state of any \texttt{System} type instance is made up of four \texttt{SystemComponent} type fields which are used to hold a reference to any system's component, that is cloud node, cloudlet node and controller; an extra \texttt{SystemComponent} type field is a short-cut used to represent the global system.

A \texttt{System} type instance has very important responsibilities including:

\begin{itemize}

\item Allocation all \texttt{SystemComponent} type objects in order to represent each system's nodes.

\item Methods implementation providing for simulation clock and system nodes state variables initialization.

\item To manage event scheduling through some methods invocations like \texttt{sche\-dule\-Event\-OnCloud}, \texttt{scheduleEventOnCloulet} etc.

\item To provide a method able to manage class 2 job interruption; the so called \texttt{removeCloud\-let\-Class2JobDeparture} method
\end{itemize}
...
A \textit{generic} system's node, like cloud or cloudlet node, is modelled by a \texttt{SystemComponent} class. 
